...
module.exports = {
 ...
 // vrne vse taske za določeno osebo
  ...
  create: async function (req, res) {
    ...
    if (name == null || start_date == null || due_date == null || description == null || users_assigned == null || project == null) {
  ...
  updateStatus: async function (req, res) {
    ...
    const foundTask = await TaskModel.findOne({ _id: taskId, author: req.user._id });
    if (!foundTask) {
      return res.status(500).json({ message: 'Error' });
    }
  ...
  delete: async function (req, res) {
    ...
    const deletedTask = await TaskModel.findOneAndDelete({ _id: taskId, author: req.user._id });
    if (deletedTask) {
